...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                ...
                // 2) fetch remote GET requests natively so the WebView is never blocked by CORS
                //    (covers the backend quote/history/news/fx and news thumbnails).
                //    POST requests (the AI providers) stay on the WebView's own stack.
                String host = request.getUrl().getHost();
                boolean isGet = "GET".equalsIgnoreCase(request.getMethod());
                if (host != null && !host.equals("appassets.androidplatform.net") && isGet) {
    ...
    private WebResourceResponse fetchNatively(String urlStr) {
        ...
        try {
            ...
            // Il dominio temporaneo *.hostingersite.com presenta una catena SSL che il
            // client nativo di Android rifiuta (il browser e' piu' permissivo). Accetta il
            // certificato SOLO per quell'host. Su un dominio tuo con SSL valido non serve.
            String h = url.getHost();
            if (c instanceof javax.net.ssl.HttpsURLConnection && h != null && h.contains("hostingersite.com")) {
                javax.net.ssl.HttpsURLConnection https = (javax.net.ssl.HttpsURLConnection) c;
